...
class TicketSummary(object):
	...
	def get_common_filters(self):
		...
		for entry in ["status", "priority", "contact", "ticket_type"]:
			...
			if self.filters.get("query_type"):
				filters["fsl_query_type"] = self.filters.get("query_type")
			if self.filters.get("call_type"):
				filters["fsl_call_type"] = self.filters.get("call_type")
			if self.filters.get("ticket_type"):
				filters["ticket_type"] = self.filters.get("ticket_type")
			if self.filters.get("agent_group"):
				filters["agent_group"] = self.filters.get("agent_group")
			if not self.filters.get("agent_group") and "Agent Team Head" in user_roles:
				# hd_team_list = frappe.db.get_list("HD Team",filters=[user,["in","users"]])
				# hd_team_list = frappe.get_all("HD Team",filters={"users":["in",[user]]})
				# all_hd_team_docs = frappe.get_all("HD Team")
				# hd_team_list = [doc for doc in all_hd_team_docs if doc["users"] == user]
				hd_team_list =frappe.get_all("HD Team", fields=["name", "users"])
				filtered_hd_team_list = [doc for doc in hd_team_list if user in doc.get("users")]


			
				frappe.msgprint(str(hd_team_list))
				filters["agent_group"] = ["in",[hd_team_list]]
	...
	def get_rows(self):
		...
		for entity, data in iteritems(self.ticket_summary_data):
			if self.filters.based_on == "Customer":
				if not entity == "Not Specified":
					name= frappe.get_doc("Customer",entity)
					row = {"customer": entity,"fsl_customer_name":name.customer_name}
				# except:
				# row={"customer": entity}


			# if self.filters.based_on == "Customer":
			# 	row = {"customer_name": entity}
				# row = {"fsl_customer_name": entity}
			elif self.filters.based_on == "Assigned To":
				row = {"user": entity}
				...
				row = {"priority": entity}
			# elif self.filters.based_on == "Customer":
			# 	row = {"fsl_customer_name": entity}
			for status in self.statuses:
