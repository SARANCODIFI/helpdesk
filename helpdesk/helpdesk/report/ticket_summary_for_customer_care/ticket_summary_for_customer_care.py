...
class TicketSummary(object):
	...
	def get_columns(self):
		...
		elif self.filters.based_on == "Branch":   
			self.columns.append(
				{
					"label": _("Branch"),
					"fieldname": "branch",
		...
		if self.filters.based_on == "Agent Group":
			self.columns.append(
				{
					"label": _("Agent Group"),
		...
		self.statuses = ["Open", "Replied", "Resolved", "Closed","Cancel"]
	...
	def get_common_filters(self):
		...
		if self.filters.get("source"):
			filters["fsl_source"] = self.filters.get("source")

		if self.filters.get("branch"):
			filters["fsl_branch_id"] = self.filters.get("branch")
	
		for entry in ["status", "priority", "customer", "ticket_type"]:
			if self.filters.get(entry):
				filters[entry] = self.filters.get(entry)
	
		allowed_sources = ["Portal", "Web", "Mobile", "Mail"]
		filters["fsl_source"] = ("in", allowed_sources)
	...
	def get_summary_data(self):
		...
		for d in self.entries:
			...
			if self.filters.based_on == "Assigned To":
				...
				frappe.errprint("im in 1st else")
		...
		if ticket:
			if self.filters.based_on == "Assigned To":
				...
				if field is not None:
					...
					frappe.throw("Invalid field specified in filters")

				for entry in data:
					value = entry.get(field)
					frappe.errprint("Im in 2nd else")
					if not value:
						value = _("Not Specified")
					...
					self.ticket_summary_data[value]["first_response_time"] = (
