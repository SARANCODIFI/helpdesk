<template>
  <div class="flex flex-col">
    <PageTitle title="Tickets">
      <template #right>
        <div class="flex gap-2">
          ...
          <RouterLink
            v-if="!configStore.preferKnowledgeBase"
            :to="{ name: CUSTOMER_PORTAL_NEW_TICKET_INTERNAL }"
          >
            <Button
              class="bg-gray-900 text-white hover:bg-gray-800"
              label="New Internal ticket"
              icon-right="plus"
            />
          </RouterLink>
        </div>
    ...
    <ListView
      :columns="columns"
      :resource="findIsBranchEmp() ? employeeTickets : tickets"
      class="mt-2.5"
      doctype="HD Ticket"
    >
...
import {
  ...
  CUSTOMER_PORTAL_NEW_TICKET_INTERNAL,
} from "@/router";
...
const tickets = createListManager({
  doctype: "HD Ticket",
  pageLength: 20,
  fields: ["name", "creation", "subject", "status","owner"],
  auto: true,
  transform: (data) => {
    for (const d of data) {
      d.onClick = {
