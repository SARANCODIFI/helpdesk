...
function submit() {
  ...
  if (email.value) {
    localStorage.setItem('userMailId', email.value)
  }
  getEmployeeDetails(email.value)
}

function getEmployeeDetails(email:any) {
    const data = fetch(`https://erp.cholasecurities.com/api/method/helpdesk.helpdesk.doctype.hd_ticket.api.get_employee?email_id=${email}`);
    data.then(response => {
      if (response.ok) {
