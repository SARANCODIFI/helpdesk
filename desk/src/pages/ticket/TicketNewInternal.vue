      ...
      <div class="flex w-full">
        <div class="w-[80%]">
          ...
          <!-- <div class="flex gap-2 items-center m-5">
            <input id="internalTicketCB" type="checkbox" v-model="isInternalTicket" class="rounded-[4px] focus:outline-none">
            <label for="internalTicketCB" class="text-xs text-gray-600">Internal Ticket</label>
          </div> -->
  
          <div v-if="!isInternalTicket" class="flex flex-col gap-1 m-5">
          ...
          <div v-if="isInternalTicket" class="flex flex-col gap-1 m-5">
            ...
            <div class="relative">
              <TextInput :value="`${employeeDetails?.data?.emp_name} - ${employeeDetails?.data?.emp_id} - ${employeeMailId}`" :type="'text'" size="sm" variant="subtle" :disabled="true"/>
            </div>
          </div>
        ...
        <div class="flex flex-col gap-2 p-5" v-if="!isInternalTicket">
        ...
        <div class="flex flex-col gap-2 p-5" v-if="isInternalTicket">
          <div class="flex flex-col gap-1">
            <div class="text-xs text-gray-600">Employee Name</div>
            <div class="font-medium text-gray-800">{{ employeeDetails?.data?.emp_name }}</div>
          </div>

          <div class="flex flex-col gap-1">
            <div class="text-xs text-gray-600">Employee ID</div>
            <div class="font-medium text-gray-800">{{ employeeDetails?.data?.emp_id }}</div>
          </div>
  
          <div class="flex flex-col gap-2">
            <div class="text-xs text-gray-600">Email ID</div>
            <div class="font-medium text-gray-800">{{ employeeMailId }}</div>
          </div>
        </div>
      </div>
      <div class="m-5">
      ...
      <span class="mx-5 mb-5">
        <TicketTextEditor ref="editor" v-model:attachments="attachments" v-model:content="description"
          ...
          <template #bottom-right>
            <Button label="Submit" theme="gray" variant="solid" :disabled="$refs.editor.editor.isEmpty || ticket.loading || !subject || !employeeMailId
              " @click="() => ticket.submit()" />
          </template>
        </TicketTextEditor>
      </span>
...
import { useAuthStore } from "@/stores/auth";
  ...
  const ticket = createResource({
    ...
    makeParams: () => ({
      doc: {
        fsl_employee_id : employeeDetails.value?.data?.emp_id,
        fsl_internal_ticket: 1,
  ...
  function getCustomers(customer) {
    ...
    if (customer.length > 2) {
      let search = customer
      const data = fetch(`https://erp.cholasecurities.com/api/method/helpdesk.overrides.search_customers?query=${search}`);
